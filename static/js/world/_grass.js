...
//Number of blades
var instances = 10 * 1000; // original 40k
if(mobile){
	instances = 7000;
	width = 50;
}
...
var elevation = 0.2; // no diff
//Rotation around Y axis in range [0, 2*PI]
var azimuth = 0.4; // no diff

var fogFade = 0.009; // no diff

//Lighting variables for grass
THREEPRESS.grass_vars = {}
// var ambientStrength = 
THREEPRESS.grass_vars.ambientStrength = 5.7;
// var translucencyStrength = 
THREEPRESS.grass_vars.translucencyStrength = 1.5;
// var specularStrength = 
THREEPRESS.grass_vars.specularStrength = 0.5;
// var diffuseStrength = 
THREEPRESS.grass_vars.diffuseStrength = 1.5;
// var shininess = 
THREEPRESS.grass_vars.shininess = 256;
// var sunColour = 
THREEPRESS.grass_vars.sunColour = new Vector3(1.0, 1.0, 1.0);
// var specularColour = 
THREEPRESS.grass_vars.specularColour = new Vector3(1.0, 1.0, 1.0);
...
// var camera = new PerspectiveCamera(FOV, window.innerWidth / window.innerHeight, 1, 20000);
...
const backgroundMaterial = new ShaderMaterial({
	uniforms: {
		sunDirection: {type: 'vec3', value: new Vector3(Math.sin(azimuth), Math.sin(elevation), -Math.cos(azimuth))},
		resolution: {type: 'vec2', value: new Vector2( RENDERER.domElement.width, RENDERER.domElement.height )},
...
var backgroundGeometry = new PlaneBufferGeometry(2, 2, 1, 1);
...
var groundBaseGeometry = new PlaneBufferGeometry(width, width, resolution, resolution);
groundBaseGeometry.lookAt(new Vector3(0,1,0));
groundBaseGeometry.verticesNeedUpdate = true;

var groundGeometry = new PlaneBufferGeometry(width, width, resolution, resolution);
...
var groundMaterial = new MeshPhongMaterial({color: 0x000900});
...
var grassBaseGeometry = new PlaneBufferGeometry(bladeWidth, bladeHeight, 1, joints);
...
var grassMaterial = new RawShaderMaterial( {
  uniforms: {
    ...
    noiseTexture: { value: noiseTexture},
		...
		ambientStrength: {type: 'float', value: THREEPRESS.grass_vars.ambientStrength},
    translucencyStrength: {type: 'float', value: THREEPRESS.grass_vars.translucencyStrength},
    diffuseStrength: {type: 'float', value: THREEPRESS.grass_vars.diffuseStrength},
    specularStrength: {type: 'float', value: THREEPRESS.grass_vars.specularStrength},
    shininess: {type: 'float', value: THREEPRESS.grass_vars.shininess},
    lightColour: {type: 'vec3', value: THREEPRESS.grass_vars.sunColour},
    specularColour: {type: 'vec3', value: THREEPRESS.grass_vars.specularColour},
  },
...
// 	var sunDirection = new Vector3(Math.sin(azimuth), Math.sin(elevation), -Math.cos(azimuth));
